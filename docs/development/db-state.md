...
Removed all test/fictitious data from the database in preparation for loading real historical data.

**Step 1 — Deleted test seasons:**
- `DELETE FROM seasons WHERE year IN (9999, 2099)` (ids 3 and 4)
- Cascaded via migration 023 constraints to: 254 `players`, 127 `player_prices`, 56 `draft_picks`, 14 `draft_order`, 2 `draft_sessions`, 2 `season_rules`, 1 `manager_round_squads`, 16 `manager_round_squad_players`

**Steps 2–4 — Deleted generated test data from 2026 season (id=1):**
- 240 `manager_round_squad_players` — test squads generated by tooling, not real manager picks
- 15 `manager_round_squads` — same
- 42 `manager_match_scores` — calculated from generated test squads
- 7 `season_standings` — derived from generated scores

**Verified post-cleanup state for season_id=1:**
| Table | Count | Note |
|---|---|---|
| `matches` | 15 | Kept — real 2026 Six Nations fixtures |
| `players` | 127 | Kept — real player pool |
| `draft_picks` | 28 | Kept — real draft picks |
| `player_match_scores` | 196 | Kept — W1+W2 real scores |
| `manager_round_squads` | 0 | Cleared |
| `manager_match_scores` | 0 | Cleared |
