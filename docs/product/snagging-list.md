...
- [ ] Comprehensive status indicators on Home/Dashboard — show clear status for all the things a manager cares about each round: squad submitted (yes/no, with deadline countdown if not); predos entered (yes/no, with deadline countdown if not); team sheets available for this round (yes/no, link to view if yes); round results/scores available yet; any other actionable items needing attention. Goal: a manager should be able to glance at the homepage or dashboard and immediately know what they need to do next, with one-click access to do it. Consider a unified "action items" or "to-do" style panel that surfaces only relevant pending actions (hide completed ones, or show them with a tick).
- [ ] Predos page UX improvements — (1) the default predos page should open to the current season and live round rather than requiring manual selection; (2) once a round has kicked off and the deadline has passed, that round's predos page should display a clear locked state showing the manager's submitted predictions as read-only, rather than showing an open entry form.
...
- [ ] Consistent season/round defaults across all pages — all pages that display season- or round-specific data should default to the active season and current live round on load, without requiring manual selection. Audit all pages (standings, squad builder, team sheets, insights, H2H, players, scores) and ensure the default selection logic is consistent. Predos default is already tracked separately above.
...
- [ ] Admin scores page — "Close round" one-click action: save predo results, calculate scores, calculate predo scores, and generate insights in a single button press, with a progress indicator showing each step completing in sequence. Also enhance the page's round status display so it's always clear what has been done (scores calculated, predos scored, insights generated, round finalised) and what the next action should be — e.g. a step-by-step checklist or pipeline view per round rather than relying on the current provisional/final badge alone.
